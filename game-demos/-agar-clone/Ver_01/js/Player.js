(function(){
	var Player = window.Player = function(name, playerMass, X, Y){
		...
		// this.x = game.global.screenWidth / 2 - this.r;
		// this.y = game.global.screenHeight / 2 - this.r;
		this.x = X - this.r;
		this.y = Y - this.r;
		this.dx = 0;
		this.dy = 0;
...
/*	Player.prototype.moveLoop = function(){
		for(var i = 0; i < game.playerArr.length; i++){
			this.playerMove(game.playerArr[i]);
		}
	}*/
	...
	Player.prototype.playerMove = function(player){
		var deg = Math.atan2(player.targetY, player.targetX);
		var diff = 10 ;
		this.dx = (this.targetX - this.x) * diff;
		this.dy = (this.targetY - this.y) * diff;
